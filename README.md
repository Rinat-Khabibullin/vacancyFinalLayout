# Практическая задача 5.2.5  
## Проверяемое задание: «URL-параметры и search-параметры»

- **Деплой:**  
  https://rinat-khabibullin.github.io/jobVacancyWebsite/

- **Репозиторий:**  
  https://github.com/Rinat-Khabibullin/jobVacancyWebsite

##  Что реализовано
- Маршрутизация приложения
- Страница вакансий
- Страница детальной вакансии (`/vacancies/:id`)
- Чтение и обработка search-параметров
- Передача параметров через URL для фильтрации/поиска
##  Также для второй практической добавил функционал
- Переключение фильтра города через табы(повторное нажатие его деактивирует и фильтр убирается)
- Добавил NotFoundPage, фото прилагаю

Тесты проходят <img width="972" height="332" alt="Снимок экрана 2026-02-07 в 19 39 34" src="https://github.com/user-attachments/assets/e09bd1b6-2777-484e-822c-a38e17a40379" />

**Страницу notFoundPage вызываем в консоли, пушая в хистори стейт:**
window.history.pushState({}, '', '/jobVacancyWebsite/does-not-exist')
window.dispatchEvent(new PopStateEvent('popstate'))
<img width="600" height="332" alt="Снимок экрана 2026-02-07 в 18 38 31" src="https://github.com/user-attachments/assets/e8a10024-4783-4a26-8540-75a75b84c3b2" />
